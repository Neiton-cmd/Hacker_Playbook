...
`Get-DomainForeignGroupMember` # PowerView script that enumerates groups with users outside of the domain and returns each foreign member. Performed from a Windows-based host.
...
`crackmapexec smb -L | grep gpp` # Locates and retrieves a group policy preference password using
